...
.then(data => {
    ...
    data.forEach(post => {
        ...
        let row = `
                <tr class="p-3"> 
                    <td class="px-3 py-3 align-middle"><p><a href="posteos?id=${id}">${post.title}</a></p><p class="d-md-none d-sm-block">${localDate} por <a href="${id}">${post.postedBy.username}</a></p></td>
